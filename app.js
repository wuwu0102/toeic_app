...
async function quickLookup(word){const normalized=String(word||'').trim().toLowerCase();if(!normalized)return'無翻譯';const local=getLibraryWords().find(w=>String(w?.word||'').toLowerCase()===normalized);if(local&&local.meaning)return local.meaning;const settings=readSettings();const api=(settings.apiBaseUrl||window.APP_CONFIG?.API_BASE_URL||'').trim().replace(/\/+$/,'');if(!api)throw new Error('lookup api missing');const res=await fetch(api+'/lookup-word',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({word:normalized})});if(!res.ok)throw new Error('HTTP '+res.status);const data=await res.json();return data.meaning||'無翻譯'}
function bindClickableWords(){document.querySelectorAll('.click-word').forEach(el=>{el.addEventListener('click',async()=>{const word=el.dataset.word;showTooltip(el,'查詢中...');try{const meaning=await quickLookup(word);showTooltip(el,meaning)}catch{showTooltip(el,'查無資料')}})})}
