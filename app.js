...
async function getWordMeaning(word){const normalized=String(word||'').trim().toLowerCase();if(!normalized)return'（未收錄）';const lib=getLibraryWords();const found=lib.find(w=>String(w?.word||'').toLowerCase()===normalized);if(found&&found.meaning)return found.meaning;try{const settings=readSettings();const api=(settings.apiBaseUrl||window.APP_CONFIG?.API_BASE_URL||'').trim().replace(/\/+$/,'');const endpoint=(api?api:'')+'/api/quick-translate';const res=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({word:normalized})});if(!res.ok)throw new Error('HTTP '+res.status);const data=await res.json();if(data&&data.zh)return data.zh}catch(e){console.warn('AI translate failed',e)}return'（未收錄）'}
