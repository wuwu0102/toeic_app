...
function continueFreePractice(){const s=ensureTodaySession();const ids=pickFreePracticeIds();if(!ids.length)return;for(const id of ids){const w=state.words[id];w.todayScore=0;w.todayWrong=0;w.todayCorrect=0}s.activeIds=ids;s.stage='quiz';s.mode='free';s.cardIndex=ids.length;s.completedQuiz=[];progressState.mode='free';progressState.dailyProgress=getTodayMasteredCount();saveState(state,{autoSync:true});saveProgress();go('quiz')}
function ensureTodaySession(){const t=todayStr();if(state.sessions[t]){state.sessions[t].mode=state.sessions[t].mode||progressState.mode||'daily';state.sessions[t].quizMode=state.sessions[t].quizMode||'practice';state.sessions[t].recentQuizIds=Array.isArray(state.sessions[t].recentQuizIds)?state.sessions[t].recentQuizIds:[];return state.sessions[t]}const ids=createDailyCandidateIds();for(const id of ids){const w=state.words[id];w.todayScore=0;w.todayWrong=0;w.todayCorrect=0}const s={date:t,activeIds:ids,stage:'cards',mode:progressState.mode||'daily',quizMode:'practice',cardIndex:0,completedCards:[],completedQuiz:[],wrongIds:[],recentQuizIds:[],stats:{green:0,yellow:0,red:0,correct:0,wrong:0}};state.sessions[t]=s;saveState(state,{autoSync:true});return s}
...
function renderStudy(){const s=ensureTodaySession();setTitle('單字卡',true);setNav('');const total=s.activeIds.length;if(s.cardIndex>=total){s.stage='quiz';saveState(state,{autoSync:true});go('quiz');return}const id=s.activeIds[s.cardIndex],w=state.words[id],exampleEn=getExampleEn(w),exampleZh=getExampleZh(w);document.getElementById('view').innerHTML=`<div class="card hero"><div class="small">第 ${s.cardIndex+1} / ${total} 張</div><div class="track"><div class="bar" style="width:${Math.round(s.cardIndex/total*100)}%"></div></div></div><div class="card"><div class="word">${w.word}</div><div class="phon">${w.phonetic}</div><div class="meta"><span class="badge">${w.pos}</span><span class="badge">${w.topic}</span><span class="badge">L${w.level}</span></div><div class="meaning">${w.meaning}</div><div class="stack" style="margin-top:12px"><button class="btn secondary" id="speakBtn" type="button">🔊 聽發音</button><button class="btn secondary" id="playSentence" type="button">🔊 聽例句</button></div><div class="exen">${exampleEn}</div><div class="exzh">${exampleZh}</div><div class="answers"><button class="btn red" data-grade="red" type="button">不會</button><button class="btn yellow" data-grade="yellow" type="button">模糊</button><button class="btn green" data-grade="green" type="button">記得</button></div></div>`;document.getElementById('speakBtn').addEventListener('click',()=>speakWord(w.word));bindSentenceTTS(w);document.querySelectorAll('[data-grade]').forEach(btn=>btn.addEventListener('click',()=>{const g=btn.dataset.grade;scheduleByCard(w,g);updateProgressAfterAnswer(w,g!=='red');if(!s.completedCards.includes(id))s.completedCards.push(id);s.stats[g]+=1;s.cardIndex+=1;saveState(state,{autoSync:true});renderStudy()}))}
...
function renderQuiz(){const s=ensureTodaySession();setTitle('熟練練習',true);setNav('');const nextId=pickNextQuizId(s);if(nextId==null){s.stage='done';s.mode='free';progressState.mode='free';progressState.dailyProgress=getTodayMasteredCount();saveState(state,{autoSync:true});saveProgress();finalizeSessionIfNeeded();go('summary');return}const w=state.words[nextId],remain=getUnmasteredCount(s),masked=makeQuestionSentence(w),exampleZh=getExampleZh(w),isExam=s.quizMode==='exam';document.getElementById('view').innerHTML=`<div class="card hero"><div class="small">今天剩餘未熟 ${remain} 題</div><div class="track"><div class="bar" style="width:${Math.round((s.activeIds.length-remain)/s.activeIds.length*100)}%"></div></div></div><div class="filter"><button class="btn ${!isExam?'primary':'light'}" id="practiceModeBtn" type="button">練習模式</button><button class="btn ${isExam?'primary':'light'}" id="examModeBtn" type="button">考試模式</button></div><div class="card quiz"><div class="exen">${masked}</div><div class="meta">${w.pos} ｜ ${isExam?'中文已隱藏':`已熟練 ${w.todayScore||0}/${DAILY_MASTER_TARGET}`}</div><button class="btn secondary" id="speakBtn" type="button">🔊 聽發音</button><input id="answerInput" class="input" placeholder="輸入英文單字" autocomplete="off" autocapitalize="none" spellcheck="false"><div class="stack" style="margin-top:10px"><button class="btn primary" id="checkBtn" type="button">確認答案</button>${!isExam?'<button class="btn light" id="revealBtn" type="button">顯示答案 / 看中文</button>':''}</div><div id="resultBox"></div></div>`;document.getElementById('speakBtn').addEventListener('click',()=>speakWord(w.word));document.getElementById('practiceModeBtn').addEventListener('click',()=>{s.quizMode='practice';saveState(state);renderQuiz()});document.getElementById('examModeBtn').addEventListener('click',()=>{s.quizMode='exam';saveState(state);renderQuiz()});const input=document.getElementById('answerInput');input.focus();function nextStep(){if(!s.completedQuiz.includes(nextId))s.completedQuiz.push(nextId);s.recentQuizIds=[...(s.recentQuizIds||[]),nextId].slice(-4);state.words[nextId].lastAskedAt=isoNow();saveState(state);renderQuiz();window.scrollTo({top:0,behavior:'instant'})}function showResult(ok){const box=document.getElementById('resultBox');if(ok){s.stats.correct+=1;box.innerHTML=`<div class="result good"><div><strong>答對了</strong></div><div style="margin-top:4px">${w.word}</div><div style="margin-top:4px">${w.meaning||'—'}</div><div class="muted" style="margin-top:4px">${exampleZh}</div><div class="muted" style="margin-top:4px">今日熟練進度：${w.todayScore}/${DAILY_MASTER_TARGET}</div></div><div class="stack" style="margin-top:10px"><button class="btn primary" id="nextBtn" type="button">下一題</button></div>`}else{s.stats.wrong+=1;if(!s.wrongIds.includes(nextId))s.wrongIds.push(nextId);box.innerHTML=`<div class="result bad"><div><strong>答錯了</strong></div><div style="margin-top:4px">正確答案：<strong>${w.word}</strong></div><div style="margin-top:4px">${w.meaning||'—'}</div><div class="muted" style="margin-top:4px">${exampleZh}</div><div class="muted" style="margin-top:4px">這個單字今天會再次出現，直到熟練。</div></div><div class="stack" style="margin-top:10px"><button class="btn primary" id="nextBtn" type="button">繼續練習</button></div>`}document.getElementById('nextBtn').addEventListener('click',nextStep)}function check(){const answer=input.value.trim().toLowerCase(),correct=w.word.toLowerCase(),ok=answer===correct;scheduleByQuiz(w,ok);updateProgressAfterAnswer(w,ok);document.getElementById('checkBtn').disabled=true;input.disabled=true;showResult(ok);saveState(state,{autoSync:true})}document.getElementById('checkBtn').addEventListener('click',check);const reveal=document.getElementById('revealBtn');if(reveal)reveal.addEventListener('click',()=>{const box=document.getElementById('resultBox');box.innerHTML=`<div class="result good"><div><strong>${w.word}</strong></div><div style="margin-top:4px">${w.meaning||'—'}</div><div class="muted" style="margin-top:4px">${exampleZh}</div></div>`});input.addEventListener('keydown',e=>{if(e.key==='Enter'&&!document.getElementById('checkBtn').disabled)check()})}
...
function hydrateProgressState(){const all=getStudyWords();const learnedWords=all.filter(w=>w.seen).map(w=>w.word.toLowerCase());if(!progressState.learnedWords.length)progressState.learnedWords=learnedWords;progressState.correctCount=Math.max(progressState.correctCount,(state.history||[]).reduce((sum,h)=>sum+(Number(h.correct)||0),0));progressState.wrongCount=Math.max(progressState.wrongCount,(state.history||[]).reduce((sum,h)=>sum+(Number(h.wrong)||0),0));progressState.mode=progressState.mode==='free'?'free':'daily';progressState.dailyProgress=getTodayMasteredCount();saveProgress()}
