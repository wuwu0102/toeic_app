...
const enMatch=text.match(/EN:\s*([\s\S]*?)\r?\nZH:/);
const zhMatch=text.match(/ZH:\s*([\s\S]*)$/);
if(!enMatch||!zhMatch){wordObj.example=originalExample;wordObj.example_zh=originalExampleZh;alert('AI 回傳格式錯誤');return}
const newEn=enMatch[1].trim();
const newZh=zhMatch[1].trim();
if(!newEn||!newZh){wordObj.example=originalExample;wordObj.example_zh=originalExampleZh;alert('AI 沒有產生完整例句');return}
wordObj.example=newEn;
wordObj.example_zh=newZh;
