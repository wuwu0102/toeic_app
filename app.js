...
async function getWordMeaning(word){const normalized=String(word||'').trim().toLowerCase();if(!normalized)return'未收錄';if(WORD_LOOKUP_CACHE[normalized])return WORD_LOOKUP_CACHE[normalized];const lib=getLibraryWords();const found=lib.find(w=>String(w?.word||'').toLowerCase()===normalized);if(found&&found.meaning){WORD_LOOKUP_CACHE[normalized]=found.meaning;return found.meaning}const settings=readSettings();const api=(settings.apiBaseUrl||window.APP_CONFIG?.API_BASE_URL||'').trim().replace(/\/+$/,'');if(!api)return'未收錄';try{const res=await fetchWithTimeout(api+'/lookup-word',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({word:normalized})},5000);if(!res.ok)throw new Error('HTTP '+res.status);const data=await res.json();const meaning=String(data?.meaning||'').trim();if(meaning){WORD_LOOKUP_CACHE[normalized]=meaning;return meaning}}catch(e){console.warn('word lookup failed',e)}return'未收錄'}
